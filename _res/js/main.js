(_ => {
	...
	window.onload = function () {
		...
		const createSwitch = (config = {}) => {
			const switchEle = createElement(`<div class="${formatClassName("switch", config.checked && "switch-checked", config.disabled && "switch-disabled")}"><input type="checkbox" class="switch-checkbox" aria-label="Toggle Theme"></div>`);
		...
		const sections = [
			...
			{section: "General", renderSection: _ => [
				...
				createElement(`<div class="welcome-details">If you expierence any Issues or got some Improvement Suggestions for my stuff, then feel free to <a href="https://github.com/mwittrien/BetterDiscordAddons/issues" target="_blank">create an issue</a>. But please make sure to check if there is already an open Thread for your Issue or Suggestion.</div>`),
				createElement(`<div class="divider"></div>`),
