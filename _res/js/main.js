	...
	const isMobile = (_) => {
		...
		if (!isM) {
			...
			document.documentElement.classList[
				window.innerHeight > window.innerWidth ? "add" : "remove"
			]("portrait");
		return isM;
	};
	isMobile();
	...
	window.onload = () => {
		...
		const createElement = (html) => {
			...
			if (template.content.childNodes.length === 1)
				return (
					template.content.firstElementChild || template.content.firstChild
				);
		...
		const toggleElement = (node, force) => {
			let hide;
			if (force === undefined) {
				hide = getComputedStyle(node, null).getPropertyValue("display") !== "none";
			} else {
				hide = !force;
			}
			if (hide) {
		...
		const appendModal = (modal, config = {}) => {
			...
			if (!modals || !(modal instanceof Node)) return;

			const wrapper = createElement(
				`<div class="modal-wrapper"><div class="backdrop"></div></div>`,
			);
		...
		const createSpinner = (wrap) => {
			return createElement(
				`${wrap ? '<div class="spinner-container">' : ""}<span class="spinner"><span class="spinner-inner"><span class="spinner-item"></span><span class="spinner-item"></span></span></span>${wrap ? "</div>" : ""}`,
			);
		};

		const createSwitch = (config = {}) => {
			const switchEle = createElement(
				`<div class="${formatClassName("switch", config.checked && "switch-checked", config.disabled && "switch-disabled")}"><input type="checkbox" class="switch-checkbox" aria-label="Toggle Theme"></div>`,
			);
			switchEle.addEventListener("click", (_) => {
				const checked = !switchEle.classList.contains("switch-checked");
		...
		const createButton = (config = {}) => {
			const button =
				createElement(`<button type="button" class="${formatClassName(config.className, "button", config.color ? `button-${config.color}` : "button-brand", config.size === "none" ? "" : config.size ? `button-size-${config.size}` : "button-size-small")}">
			...
			invite.querySelector(".invite-content").appendChild(
				createButton({
					...
					onClick: (_) => {
						data.invite &&
							window.open(
								`https://discord.com/invite/${data.invite}`,
								"_blank",
							);
					},
				}),
		...
		const createTooltip = (anker, text, config = {}) => {
			...
			const itemLayer =
				createElement(`<div class="layer-item layer-item-disabled">
				<div class="tooltip">
			...
			tooltip.classList.add(
				config.color ? `tooltip-${config.color}` : "tooltip-primary",
			);
			...
			const wheel = (e) => {
				...
				wheel.timeout = setTimeout((_) => {
					...
					if (tRects1.x !== tRects2.x || tRects1.y !== tRects2.y)
						removeTooltip();
				}, 500);
			};
			const mouseMove = (e) => {
				const parent = e.target.parentElement.querySelector(":hover");
				if (parent && anker !== parent && !anker.contains(parent)) {
				...
				switch (type) {
					...
					case "bottom":
						...
						case "left":
							...
							left =
								tRects.left - iRects.width - positionOffsets.width + 2 - offset;
							break;
						case "right":
							top = tRects.top + (tRects.height - iRects.height) / 2;
							left =
								tRects.left + tRects.width + positionOffsets.width - 2 + offset;
							break;
					}
					...
					if (type === "top" || type === "bottom") {
						if (left < 0) {
							...
							tooltipPointer.style.setProperty(
								"margin-left",
								`${left - 10}px`,
								"important",
							);
		...
		const createLoadingList = (url, config = {}) => {
			...
			header.update = (amount) => {
				const hadFocus =
					...
					header.querySelector("input") === document.activeElement;
				while (header.childElementCount) header.firstElementChild.remove();
				const children = [
					typeof config.getHeader === "function"
						? config.getHeader(amount)
						: config.getHeader,
				]
					.flat(10)
					.filter((n) => n);
			...
			list.update = (_) => {
				try {
					...
					const children = [
						typeof config.getChildren === "function"
							? config.getChildren(createLoadingList.cache[url])
							: config.getChildren,
					]
						.flat(10)
						.filter((n) => n);
		...
		const createAddonList = (type, ending) => {
			const [header, list] = createLoadingList(
				`https://mwittrien.github.io/BetterDiscordAddons/${type}/README.md`,
				{
					...
					getHeader: (amount) => {
						...
						const updateSearch = (newSearch) => {
							...
							const hasSearch =
								createAddonList.search[type] &&
								createAddonList.search[type].length > 0;
							icon.classList[hasSearch ? "add" : "remove"](
								"search-icon-layout-clear",
							);
							mag.classList[!hasSearch ? "add" : "remove"](
								"search-icon-visibile",
							);
							clear.classList[hasSearch ? "add" : "remove"](
								"search-icon-visibile",
							);
						};
						let timeout;
						...
						return [
							createElement(
								`<div class="addon-list-title">${type} - ${amount}</div>`,
							),
							search,
						];
					},
					getChildren: (response) =>
						response
							...
							.map((n) => {
								const m = n.split("](");
								const l = m[1].split(") - ");
								const fileName = l[0].split("/").pop();
								const name = (m[0] || "").trim();
								const description = (l[1] || "").trim();
								return (
									name &&
									(!createAddonList.search[type] ||
										[name, description]
											...
											.some(
												(s) =>
													s
														.toLowerCase()
														.indexOf(
															createAddonList.search[type].toLowerCase(),
														) > -1,
											)) &&
									createAddonCard({
										name: name,
		...
		const createLibraryEntry = (_) => {
			return createLoadingList(
				"https://mwittrien.github.io/BetterDiscordAddons/Library/README.md",
				{
					className: "addon-list",
					getHeader: "Required Library",
					getChildren: (response) => {
						const parsedResponse = response.trim().replace(/\r/g, "");
						const fileName = "BDFDB";
						return createAddonCard({
							...
							resourceUrl:
								"https://mwittrien.github.io/BetterDiscordAddons/Library/_res",
							websiteUrl:
								"https://github.com/mwittrien/BetterDiscordAddons/tree/master/Library",
		...
		const createAddonCard = (addon) => {
			const card = createElement(`<div class="addon-card">
			...
			const headerEle = card.querySelector(".addon-header");
			const linksEle = card.querySelector(".addon-links");
			const buttonsEle = card.querySelector(".addon-buttons");

			if (addon.resourceUrl) {
				if (!createAddonCard.cache[addon.resourceUrl])
					createAddonCard.cache[addon.resourceUrl] = {};

				const addCover = (_) => {
					if (
						createAddonCard.cache[addon.resourceUrl].cover.status !== 200 ||
						!createAddonCard.cache[addon.resourceUrl].cover.url
					)
						return;
					headerEle.classList.add("has-screenshots");
					const coverWrapper = headerEle.querySelector(".addon-cover-wrapper");
					while (coverWrapper.childElementCount)
						coverWrapper.firstElementChild.remove();
					coverWrapper.appendChild(
						createElement(
							`<img class="addon-cover" src="${createAddonCard.cache[addon.resourceUrl].cover.url}" alt="">`,
						),
					);
				};
				if (createAddonCard.cache[addon.resourceUrl].cover) addCover();
					...
					const checkCoverUrl = (_) => {
						...
						if (!type)
							createAddonCard.cache[addon.resourceUrl].cover = { status: 404 };
						else {
							const xhttp = new XMLHttpRequest();
							const url = `${addon.resourceUrl}/cover.${type}`;
							xhttp.onload = function () {
								if (this.status === 200) {
									createAddonCard.cache[addon.resourceUrl].cover = {
										status: this.status,
										url: url,
									};
				...
				headerEle.addEventListener("click", (_) => {
					if (
						...
						!createAddonCard.cache[addon.resourceUrl].screenshots ||
						(!createAddonCard.cache[addon.resourceUrl].cover.url &&
							!createAddonCard.cache[addon.resourceUrl].screenshots.urls.length)
					)
						return;
					...
					const openScreenshots = (_) => {
						const urls = [
							...new Set(
								[
									createAddonCard.cache[addon.resourceUrl].cover.url,
									createAddonCard.cache[addon.resourceUrl].screenshots.urls,
								]
									.flat(10)
									.filter((n) => n),
							),
						...
						const switchImages = (offset) => {
							...
							if (Number.isNaN(roundedOffset) || !urls[i + roundedOffset]) return;
							i += roundedOffset;
						...
						const loadImage = (screenshot, url) => {
							...
							if (url) {
								if (images[url]) setImage(screenshot, url);
									...
									const loadingIcon = createSpinner(true);
									const image = screenshot.firstElementChild;
						...
						const setImage = (screenshot, url) => {
							...
							const ratio = Math.min(
								isMobile() ? 100 : 1,
								(aRects.width * 0.8 - 20) / images[url].width,
								(aRects.height - 100) / images[url].height,
							);
					...
					if (createAddonCard.cache[addon.resourceUrl].screenshots.fetched)
						openScreenshots();
					else {
							...
							xhttp.onload = function () {
								if (this.status === 200) {
									createAddonCard.cache[
										addon.resourceUrl
									].screenshots.urls.push(url);
									loadScreenshot();
								} else {
									createAddonCard.cache[addon.resourceUrl].screenshots.fetched =
										true;
			...
			for (const link of links) {
				const linkEle = createElement(
					`<div class="addon-link">${link.svg}</div>`,
				);
				if (link.text)
					linkEle.addEventListener("mouseenter", (_) =>
						createTooltip(linkEle, link.text),
					);
				if (link.url)
					linkEle.addEventListener("click", (_) =>
						window.open(link.url, "_blank"),
					);
				linksEle.appendChild(linkEle);
			}
			for (const button of buttons) {
				const buttonEle = createElement(
					`<button class="addon-button">${button.svg}</button>`,
				);
				if (button.text)
					buttonEle.addEventListener("mouseenter", (_) =>
						createTooltip(buttonEle, button.text),
					);
		...
		const sidebarHeader = document.querySelector(
			".sidebar-region .sidebar-header",
		);
		const sidebarFooter = document.querySelector(
			".sidebar-region .sidebar-footer",
		);
		...
		const sections = [
			{ section: "Title", title: "My Stuff" },
			{
				...
				renderSection: (_) => [
					createElement(
						`<div class="welcome-message">Welcome to my Plugins and Themes Repository</div>`,
					),
					createElement(`<div class="divider"></div>`),
					createElement(
						`<div class="welcome-details">Here you'll find all my Plugins and Themes that are hosted on my <a href="https://github.com/mwittrien/BetterDiscordAddons/" target="_blank">GitHub</a>.</div>`,
					),
					createElement(
						`<div class="welcome-details">If you expierence any Issues or got some Improvement Suggestions for my stuff, then feel free to <a href="https://github.com/mwittrien/BetterDiscordAddons/issues" target="_blank">create an issue</a>. But please make sure to check if there is already an open Thread for your Issue or Suggestion.</div>`,
					),
					createElement(`<div class="divider"></div>`),
					createElement(
						`<div class="welcome-details">If you enjoy using my Stuff and want to support me feel free to donate to me or become a Patron and receive certain benefits depending on which Tier you choose.</div>`,
					),
					createElement(
			...
			{
				section: "Themes",
				renderSection: (_) => createAddonList("Themes", ".theme.css"),
			},
			{ section: "Separator" },
			{
				section: "Invites",
				renderSection: (_) => [
					createInvite({
						invite: "Jx3TjNS",
						name: "DevilBro's Haus",
						details: "Support Server for all my Stuff",
						icon: "https://mwittrien.github.io/_res/imgs/db_logo.webp",
					}),
					createElement(`<div class="divider"></div>`),
					createInvite({
						invite: "0Tmfo5ZbORCRqbAd",
						name: "BetterDiscord",
						details: "First Support Server for BD",
						icon: "https://mwittrien.github.io/_res/imgs/bd1_logo.webp",
					}),
					createInvite({
						invite: "sbA3xCJ",
						name: "BetterDiscord2",
						details: "Second Support Server for BD",
						icon: "https://mwittrien.github.io/_res/imgs/bd2_logo.webp",
					}),
				],
			},
		];
		...
		const renderContent = (data) => {
			...
			const sectionElements = [data.renderSection()].flat(10).filter((n) => n);
			for (const element of sectionElements) content.appendChild(element);
			...
			(parsedSection &&
				sections.find(
					(data) =>
						...
						data.section.toLowerCase() === parsedSection,
				)) ||
				sections.find(
					(data) => data.section !== "Title" && data.section !== "Separator",
				),
		);

		sidebarHeader.appendChild(
			createElement(
				`<div class="logo"><div class="primary">Better</div><div class="secondary">Discord</div></div>`,
			),
		);
			...
			const item = createElement(
				data.section === "Title"
					...
					: data.section === "Separator"
						...
						: `<div class="${formatClassName("sidebar-item", selectedSection === data.section && "selected")}">${data.section}</div>`,
			);
		...
		document.addEventListener("touchstart", (event) => {
			const trigger = isMobile() && !document.querySelector(".modals > *");
