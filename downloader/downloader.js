window.DownloadApi = {
	converter: {
		...
		url: arg => {
			if (!arg.startsWith("https://") && !arg.startsWith("http://")) {
				arg = `https://raw.githubusercontent.com/mwittrien/BetterDiscordAddons/master/${arg}`;
			}
			return arg;
		}
	},
	convert: (parameterString, error) => {
		if (typeof parameterString === "string") for (const parameter in window.DownloadApi.converter) {
			const arg = (parameterString.split(`?${parameter}=`)[1] || "").split("?")[0] || "";
			if (arg) {
	...
	download: (url, error) => {
		...
		fetch(url).then(response => {
			if (response.status === 404) error?.(`GitHub File <a href="${url}">${url}</a> does not exist!`);
